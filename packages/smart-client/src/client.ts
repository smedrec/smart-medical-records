...
import { SmartClientError, SmartClientInitializationError, SmartClientAuthenticationError, SmartClientRequestError } from './types';
...
export class SmartClient {
  ...
  constructor(config: SmartClientConfig) {
    ...
    if (config.iss !== config.clientId) {
        throw new SmartClientInitializationError('Invalid SmartClientConfig: iss and clientId must be identical for this authentication flow.');
    }
  ...
  private async initializeClient(issUrl?: string): Promise<void> {
    ...
    if (issUrl) {
      ...
      try {
        ...
        const authorityRoot = fhirBaseUrlObj.origin.endsWith('/') ? fhirBaseUrlObj.origin : `${fhirBaseUrlObj.origin}/`;
    ...
    try {
      ...
      if (
        ...
        !this.smartConfiguration.token_endpoint_auth_signing_alg_values_supported.includes(this.config.signingAlgorithm!)
      ) {
  ...
  public async getAccessToken(forceRefresh: boolean = false): Promise<string> {
    ...
    if (!forceRefresh && this.tokenResponse?.access_token && this.tokenExpiryTime && Date.now() < this.tokenExpiryTime) {
    ...
    try {
      ...
      if (!this.tokenResponse.access_token || typeof this.tokenResponse.expires_in !== 'number') {
        throw new SmartClientAuthenticationError('Received invalid token response from server: access_token or expires_in missing/invalid.', { details: this.tokenResponse });
      }
      ...
      this.tokenExpiryTime = Date.now() + (this.tokenResponse.expires_in - safetyBufferSeconds) * 1000;

      return this.tokenResponse.access_token;
    } catch (error: any) {
       const errorDetails = error.response?.data ? { responseData: error.response.data, status: error.response.status } : {};
      ...
      throw new SmartClientAuthenticationError(
        errorMessage,
        { cause: error, details: errorDetails }
      );
    }
  }
  ...
  private async createClientAssertion(): Promise<string> {
    if (!this.smartConfiguration?.token_endpoint) {
      throw new SmartClientAuthenticationError('Token endpoint URL is not available. Initialize the client by calling static init() method.');
    }
    ...
    try {
      ...
      throw new SmartClientAuthenticationError(`Failed to create or sign JWT assertion: ${error.message}`, { cause: error });
    }
  }
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response) {
          ...
          if ((axiosError.response.status === 401 || axiosError.response.status === 403) && !params?.['_retryAttempted']) {
            console.warn(`FHIR request received ${axiosError.response.status}. Attempting to refresh token and retry.`);
            try {
              ...
              if (retryError instanceof SmartClientAuthenticationError) {
                 ...
                 throw new SmartClientAuthenticationError(retryErrorMessage, { cause: retryError, details: retryError.details });
              }
              throw new SmartClientRequestError(retryErrorMessage, { cause: retryError, details: (retryError as SmartClientError).details || errorDetails});
            }
          }
        }
  ...
  private formatOperationOutcome(operationOutcome: any): string {
    if (operationOutcome?.issue && Array.isArray(operationOutcome.issue)) {
      return operationOutcome.issue
        .map((iss: any) => `[${iss.severity}/${iss.code}] ${iss.diagnostics || iss.details?.text || 'No details provided.'}`)
        .join('; ');
    }
