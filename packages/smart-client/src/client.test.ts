...
import { SmartClientInitializationError, SmartClientAuthenticationError, SmartClientRequestError } from './types';
...
describe('SmartClient', () => {
  ...
  describe('Constructor', () => {
    ...
    it('should throw SmartClientInitializationError for invalid config (missing required fields)', () => {
      ...
      invalidConfigs.forEach(config => {
        ...
        expect(() => new SmartClient(config)).toThrow('Invalid SmartClientConfig: clientId, iss, scope, and privateKey are required.');
      });
    });
    ...
    it('should throw SmartClientInitializationError if iss and clientId are not identical', () => {
        ...
        expect(() => new SmartClient(configWithMismatch)).toThrow('Invalid SmartClientConfig: iss and clientId must be identical for this authentication flow.');
    });
  ...
  describe('init() static method', () => {
    ...
    it('should throw SmartClientInitializationError if fhirBaseUrl and issUrl are missing for discovery', async () => {
        ...
        await expect(SmartClient.init(configNoUrls, undefined))
            .rejects.toThrow(SmartClientInitializationError);
        await expect(SmartClient.init(configNoUrls, undefined))
            .rejects.toThrow('Cannot determine .well-known/smart-configuration URL: issUrl or fhirBaseUrl must be provided for discovery.');
    });
    ...
    it('should throw SmartClientInitializationError if SMART config fetch fails', async () => {
      ...
      await expect(SmartClient.init(BASE_CONFIG)).rejects.toThrow('Failed to fetch SMART configuration');
    });

    it('should throw SmartClientInitializationError if SMART config is missing token_endpoint', async () => {
      mockAxiosInstance.get.mockResolvedValueOnce({ data: { ...SMART_CONFIGURATION_MOCK, token_endpoint: undefined } });
      await expect(SmartClient.init(BASE_CONFIG)).rejects.toThrow(SmartClientInitializationError);
      await expect(SmartClient.init(BASE_CONFIG)).rejects.toThrow('Invalid SMART configuration: token_endpoint is missing.');
    });

    it('should log a warning if private_key_jwt is not in token_endpoint_auth_methods_supported', async () => {
      const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      mockAxiosInstance.get.mockResolvedValueOnce({
        data: { ...SMART_CONFIGURATION_MOCK, token_endpoint_auth_methods_supported: ['client_secret_basic'] },
      });
      await SmartClient.init(BASE_CONFIG);
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("Server does not explicitly list 'private_key_jwt' in token_endpoint_auth_methods_supported")
      );
    ...
    it('should log a warning if configured signing algorithm is not in token_endpoint_auth_signing_alg_values_supported', async () => {
      const consoleWarnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      mockAxiosInstance.get.mockResolvedValueOnce({
        data: { ...SMART_CONFIGURATION_MOCK, token_endpoint_auth_signing_alg_values_supported: ['ES384'] },
      });
      // BASE_CONFIG uses RS384 by default
      await SmartClient.init(BASE_CONFIG);
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        expect.stringContaining("configured signing algorithm 'RS384' is not listed in server's token_endpoint_auth_signing_alg_values_supported")
      );
      consoleWarnSpy.mockRestore();
    });
  });
  ...
  describe('getAccessToken()', () => {
    ...
    it('should use JWK private key if provided and parsed', async () => {
        const clientWithJwk = await SmartClient.init({ ...BASE_CONFIG, privateKey: JSON.stringify(TEST_PRIVATE_KEY_JWK) });
        await clientWithJwk.getAccessToken();
        expect(importJWK).toHaveBeenCalledWith(TEST_PRIVATE_KEY_JWK, BASE_CONFIG.signingAlgorithm || 'RS384');
    ...
    it('should use PKCS8 private key if JWK parsing fails or not a JWK', async () => {
        ...
        expect(importPKCS8).toHaveBeenCalledWith(TEST_PRIVATE_KEY_PEM, BASE_CONFIG.signingAlgorithm || 'RS384');
    });
    ...
    it('should throw SmartClientAuthenticationError if token endpoint post fails', async () => {
      ...
      await expect(client.getAccessToken()).rejects.toThrow('Failed to obtain access token: Token endpoint error');
    });
    ...
    it('should throw SmartClientAuthenticationError with details from server error response', async () => {
        ...
        await expect(client.getAccessToken()).rejects.toThrow(SmartClientAuthenticationError);
        await expect(client.getAccessToken()).rejects.toThrow('Failed to obtain access token: Client authentication failed (Code: invalid_client)');
    });


    it('should throw SmartClientAuthenticationError if token response is invalid (missing access_token)', async () => {
      mockAxiosInstance.post.mockResolvedValueOnce({ data: { ...TOKEN_RESPONSE_MOCK, access_token: undefined } });
      await expect(client.getAccessToken()).rejects.toThrow(SmartClientAuthenticationError);
      await expect(client.getAccessToken()).rejects.toThrow('Received invalid token response from server: access_token or expires_in missing/invalid.');
    });

    it('should throw SmartClientAuthenticationError if token response is invalid (missing expires_in)', async () => {
      mockAxiosInstance.post.mockResolvedValueOnce({ data: { ...TOKEN_RESPONSE_MOCK, expires_in: undefined } });
      await expect(client.getAccessToken()).rejects.toThrow(SmartClientAuthenticationError);
       await expect(client.getAccessToken()).rejects.toThrow('Received invalid token response from server: access_token or expires_in missing/invalid.');
    });
    ...
    it('should throw SmartClientAuthenticationError if client is not initialized', async () => {
      ...
      await expect(uninitializedClient.getAccessToken()).rejects.toThrow(SmartClientAuthenticationError);
      await expect(uninitializedClient.getAccessToken()).rejects.toThrow('SmartClient not properly initialized or token_endpoint is missing.');
    });

    it('should use configured kid and jwksUrl in JWT header', async () => {
        const configWithKidJku = { ...BASE_CONFIG, kid: 'custom-kid', jwksUrl: 'https://client.example.com/jwks.json' };
  ...
  describe('FHIR API requests (get, post, put, delete, patch)', () => {
    ...
    const mockPatient = { resourceType: 'Patient', id: '123', name: [{ family: 'Test', given: ['User'] }] };
    ...
    it('get() should make a GET request with Authorization header', async () => {
      ...
      expect(mockAxiosInstance.request).toHaveBeenCalledWith(expect.objectContaining({
        url: resourcePath,
        method: 'GET',
        headers: expect.objectContaining({ Authorization: `Bearer ${TOKEN_RESPONSE_MOCK.access_token}` }),
      }));
    });
    ...
    it('should throw SmartClientRequestError if client not initialized (no smartConfiguration)', async () => {
        ...
        await expect(uninitializedClient.get(resourcePath)).rejects.toThrow('Client not initialized. Call static init() method first.');
    });
    ...
    it('should throw SmartClientRequestError if API request fails with non-401/403 error', async () => {
        ...
        await expect(client.get(resourcePath)).rejects.toThrow("FHIR API request to 'Patient/123' failed with status 500");
    });

    it('should format OperationOutcome in error message if present', async () => {
        const operationOutcomeError = {
            resourceType: "OperationOutcome",
            issue: [{ severity: "error", code: "processing", diagnostics: "Detailed error diagnostic message." }]
        };
        mockAxiosInstance.request.mockRejectedValueOnce({
            isAxiosError: true,
