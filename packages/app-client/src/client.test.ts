...
describe('AppClient', () => {
	...
	describe('Error Handling', () => {
		it('should throw an error if the request fails after retries', async () => {
			...
			await expect(client.ok()).rejects.toThrow(
	...
	describe('Retry Mechanism', () => {
		...
		it('should apply exponential backoff', async () => {
			vi.useFakeTimers()
			const clientWithLongerBackoff = new AppClient({
				...defaultOptions,
				retries: 2,
				backoffMs: 100,
				maxBackoffMs: 500,
		...
		it('should respect maxBackoffMs', async () => {
			vi.useFakeTimers()
			const clientWithMaxBackoff = new AppClient({
				...defaultOptions,
				retries: 3,
				backoffMs: 300,
				maxBackoffMs: 500,
			...
			global.fetch = vi
				...
				.mockResolvedValueOnce({
					...
					status: 500,
					text: () => Promise.resolve('Error 1'),
				} as Response) // delay 300ms
				.mockResolvedValueOnce({
					ok: false,
					status: 500,
					text: () => Promise.resolve('Error 2'),
				} as Response) // delay 500ms (capped from 300*2=600)
				.mockResolvedValueOnce({
					ok: false,
					status: 500,
					text: () => Promise.resolve('Error 3'),
				} as Response) // delay 500ms (capped from 500*2=1000)
				.mockResolvedValueOnce({
					ok: true,
					status: 200,
					json: () => Promise.resolve({ ok: true }),
