...
export const createSmartFhirClient = async (options: SmartFhirClientOptions): Promise<Client<paths, `${string}/${string}`>> => {
  if (!options.request) {
  ...
  if (receivedState !== mergedConfig.expectedState) {
    throw new Error(`Invalid state parameter received. Expected ${mergedConfig.expectedState} but got ${receivedState}. CSRF attack suspected.`);
  }
  ...
  if (!mergedConfig.code) {
     throw new Error("Authorization code not found in request URL. Cannot proceed with FHIR.oauth2.ready().");
  }
  ...
  try {
    ...
    console.error("FHIR.oauth2.ready() failed:", error.message || error, error.response?.data ? { data: error.response.data } : "");
    if (error.stack) console.error(error.stack);
    throw new Error(`SMART on FHIR authentication failed during ready(): ${error.message || 'Unknown error'}`);
  }
  ...
  if (!accessToken) {
    console.warn("No access token available from SMART client despite successful ready(). Potentially an issue with token response.", smartClient.state.tokenResponse);
...
export const authorizeSmartClient = async (
  ...
  if (launch) {
    ...
    if (typeof launch === 'string' && launch !== "true") { // "true" as string might be from env var
...
const defaultIssForBasicClient =
  (typeof process !== 'undefined' && process.env.SMART_ISS) ||
  (typeof process !== 'undefined' && process.env.FHIR_BASE_URL) ||
  undefined;
 ...
 */
export const _internalDefaultFhirClient = createClient(defaultIssForBasicClient ? { baseUrl: defaultIssForBasicClient } : {});
