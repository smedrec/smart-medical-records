...
export function createAllergyIntoleranceSchema<C extends z.ZodTypeAny = z.ZodUnknown>(options?: {
	contained?: C
	...
	return getCachedSchema('AllergyIntolerance', [contained], () => {
		const baseSchema: z.ZodType<types.AllergyIntolerance<z.infer<C>>> = z.strictObject({
			...
			category: z.enum(['food', 'medication', 'environment', 'biologic']).array().optional(),
...
export function createAllergyIntoleranceReactionSchema() {
	return getCachedSchema('AllergyIntoleranceReaction', [], () => {
		const baseSchema: z.ZodType<types.AllergyIntoleranceReaction> = z.strictObject({
