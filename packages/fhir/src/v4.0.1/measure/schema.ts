...
export function createMeasureSchema<C extends z.ZodTypeAny = z.ZodUnknown>(options?: {
	contained?: C
	...
	return getCachedSchema('Measure', [contained], () => {
		const baseSchema: z.ZodType<types.Measure<z.infer<C>>> = z.strictObject({
			...
			clinicalRecommendationStatement: primitives.getMarkdownSchema().optional(),
			...
			supplementalData: z.array(createMeasureSupplementalDataSchema()).optional(),
		})
...
export function createMeasureReportSchema<C extends z.ZodTypeAny = z.ZodUnknown>(options?: {
	contained?: C
	allowContained?: boolean
}) {
	const contained =
	...
	return getCachedSchema('MeasureReport', [contained], () => {
		const baseSchema: z.ZodType<types.MeasureReport<z.infer<C>>> = z.strictObject({
...
export function createMeasureReportPopulation1Schema() {
	return getCachedSchema('MeasureReportPopulation1', [], () => {
		const baseSchema: z.ZodType<types.MeasureReportPopulation1> = z.strictObject({
			id: primitives.getStringSchema().optional(),
			extension: z.array(createExtensionSchema()).optional(),
			modifierExtension: z.array(createExtensionSchema()).optional(),
			code: createCodeableConceptSchema().optional(),
