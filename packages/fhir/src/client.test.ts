...
vi.mock('fhirclient', async (importOriginal) => {
  ...
  return {
    ...
    default: { // Assuming FHIR is the default export
      ...actual.default,
      oauth2: {
...
describe('FHIR Client - Unit Tests', () => {
  ...
  describe('authorizeSmartClient', () => {
    it('should construct correct authorizeUrl and return verifier and state', async () => {
      ...
      expect(FHIR.oauth2.utils.getWellKnownSMARTConfig).toHaveBeenCalledWith(mockEnvOptions.SMART_ISS);

      const authorizeUrl = new URL(result.authorizeUrl);
      expect(authorizeUrl.origin).toBe(mockSmartConfig.authorization_endpoint.substring(0, mockSmartConfig.authorization_endpoint.indexOf('/authorize')));
    ...
    it('should throw if clientId is missing', async () => {
      const options: SmartFhirClientOptions = { env: { ...mockEnvOptions, SMART_CLIENT_ID: undefined } };
    ...
    it('should throw if iss is missing', async () => {
      ...
      await expect(authorizeSmartClient(options)).rejects.toThrow('SMART ISS (FHIR Server URL) is required');
    });
    ...
    it('should throw if getWellKnownSMARTConfig fails', async () => {
      ...
      await expect(authorizeSmartClient(options)).rejects.toThrow('Failed to fetch SMART configuration');
    });
  });
  ...
  describe('createSmartFhirClient', () => {
    ...
    beforeEach(() => {
      mockRequest = new Request('https://app.example.com/auth/callback?code=test-code&state=test-state') as Request;
    ...
    it('should successfully initialize client on valid callback', async () => {
      ...
      expect(FHIR.oauth2.ready).toHaveBeenCalledWith(expect.objectContaining({
        code: 'test-code',
        state: 'test-state',
    ...
    it('should throw if state mismatch', async () => {
      ...
      await expect(createSmartFhirClient(options)).rejects.toThrow(/Invalid state parameter received/);
    });

    it('should throw if code is missing from URL', async () => {
      const reqWithoutCode = new Request('https://app.example.com/auth/callback?state=test-state') as Request;
      const options: SmartFhirClientOptions = {
      ...
      await expect(createSmartFhirClient(options)).rejects.toThrow('Authorization code not found in request URL');
    });
    ...
    it('should throw if pkceCodeVerifier is missing', async () => {
      ...
      await expect(createSmartFhirClient(options)).rejects.toThrow('PKCE code_verifier must be provided');
    });
    ...
    it('should throw if FHIR.oauth2.ready fails', async () => {
      ...
      await expect(createSmartFhirClient(options)).rejects.toThrow(/SMART on FHIR authentication failed during ready.*Token exchange failed/);
    });

    it('should throw if no access token after ready() succeeds', async () => {
      const readyResponseWithoutToken = { ...successfulReadyResponse, state: { ...successfulReadyResponse.state, tokenResponse: { ...successfulReadyResponse.state.tokenResponse, access_token: undefined } } };
      // @ts-ignore
  ...
  describe('Crypto Utilities', () => {
    ...
    it('generatePkceChallenge produces a SHA256 hash then base64url', async () => {
      const verifier = 'test_verifier_string_long_enough_for_sha256_to_be_meaningful_and_not_trivial';
