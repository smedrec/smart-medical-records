...
export default function Home() {
	...
	const privateData = useQuery({
		queryKey: ['version'],
		queryFn: async () => await app.version(),
		enabled: true,
		staleTime: Infinity,
		refetchOnWindowFocus: false,
	})
	...
	return (
		<Container>
			<ScrollView className="flex-1">
				<View className="px-4">
					<Text className="font-mono text-foreground text-3xl font-bold mb-4">SMEDREC</Text>
					{session?.user ? (
					...
					<View className="mb-6 rounded-lg border border-border p-4">
						...
						{privateData && (
							<View>
								<Text className="text-muted-foreground">{privateData.data?.version}</Text>
							</View>
						)}
					</View>
					{!session?.user && (
