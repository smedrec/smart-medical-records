...
const route = createRoute({
	tags: ['AI'],
	operationId: 'ai-details-index',
	method: 'get',
	path: '/ai/details-index',
	security: [{ cookieAuth: [] }],
	request: {
		body: {
			required: true,
...
export type AiDetailsIndexRequest = z.infer<
	(typeof route.request.body.content)['application/json']['schema']
>
export type AiDetailsIndexResponse = z.infer<
	(typeof route.responses)[200]['content']['application/json']['schema']
>

export const registerAiDetailsIndex = (app: App) =>
    ...
    if (!canCreateAssistant) {
      throw new ApiError({
		...
		try {
			...
			const response = await vectorStore.details(data.indexName)
