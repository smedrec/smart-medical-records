...
import { therapist, user } from '@repo/db'
...
const route = createRoute({
	...
	responses: {
		200: {
			...
			content: {
				'application/json': {
					schema: z.array(TherapistSelectSchema),
				},
			},
		},
...
export type TherapistGetAllResponse = z.infer<
	(typeof route.responses)[200]['content']['application/json']['schema']
>

export const registerTherapistGetAll = (app: App) =>
	app.openapi(route, async (c) => {
		...
		const total = await db.$count(therapist, eq(therapist.organization, organization))
		...
		const result = await db
			...
			.where(eq(therapist.organization, organization))
			.orderBy(asc(therapist.id)) // order by is mandatory
