...
describe('MCP Authentication Integration Tests', () => {
	...
	describe('GET /auth/login', () => {
		it('should redirect to FHIR IdP authorization endpoint and set PKCE/state cookies', async () => {
			...
			expect(
				cookies.some(
					(c) =>
						c.startsWith(`${FHIR_PKCE_VERIFIER_COOKIE}=`) &&
						c.includes('HttpOnly') &&
						c.includes('Secure') &&
			...
			expect(
				cookies.some(
					(c) =>
						c.startsWith(`${FHIR_AUTH_STATE_COOKIE}=`) &&
						c.includes('HttpOnly') &&
						c.includes('Secure') &&
						c.includes('SameSite=Lax') &&
						c.includes('Path=/')
	...
	describe('GET /auth/callback', () => {
		...
		beforeEach(async () => {
			...
			mockFetch.mockResolvedValueOnce(
				// For .well-known in this specific beforeEach
				new Response(JSON.stringify(smartWellKnownConfig), {
					status: 200,
					headers: { 'Content-Type': 'application/json' },
				})
		...
		it('should fail if token endpoint returns an error', async () => {
			mockFetch.mockResolvedValueOnce(
				...
				new Response(
					JSON.stringify({
						error: 'invalid_grant',
						error_description: ' проблемы с кодом авторизации или кодом PKCE',
					}),
	...
	describe('GET /auth/logout', () => {
		it('should clear session cookie and redirect to /', async () => {
			...
			expect(
				cookies.some(
					(c) =>
						c.startsWith(`${FHIR_SESSION_COOKIE}=;`) &&
						(c.includes('Max-Age=0') || c.includes('expires=Thu, 01 Jan 1970'))
				)
			).toBe(true)
		})
