...
import type { Bundle, OperationOutcome, Patient } from '@solarahealth/fhir-r4'
	...
	describe('patientReadTool.handler', () => {
		...
		it('should read patient successfully when authorized', async () => {
			...
			mockFhirClient.GET.mockResolvedValue({
				data: mockPatient,
				error: null,
				response: new Response(JSON.stringify(mockPatient), { status: 200 }),
			})
			...
			expect(mockFhirClient.GET).toHaveBeenCalledWith('/Patient/{id}', {
				params: { path: { id: readParams.id } },
			})
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({ status: 'attempt', action: `${toolName}Attempt` })
			)
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({ status: 'success', action: `cerbos:read` })
			)
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({ status: 'success', action: toolName })
			)
		})
		...
		it('should throw error and log when Cerbos authorization fails', async () => {
			...
			await expect(patientReadTool.handler(readParams, mockCallContext)).rejects.toThrow(
			...
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({
					status: 'failure',
					action: `cerbos:read`,
					outcomeDescription: expect.stringContaining('Forbidden'),
				})
			)
		})
		...
		it('should throw error and log when FHIR client call fails', async () => {
			...
			await expect(patientReadTool.handler(readParams, mockCallContext)).rejects.toThrow(
				/FHIR Patient read failed: Status 404/
			)
			...
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({
					status: 'failure',
					action: toolName,
					outcomeDescription: 'FHIR Patient read failed: Status 404',
				})
			)
		})
		...
		it('should throw error if FHIR client is not available in context', async () => {
			...
			await expect(patientReadTool.handler(readParams, contextWithoutClient)).rejects.toThrow(
	...
	describe('patientCreateTool.handler', () => {
		...
		it('should create patient successfully when authorized', async () => {
			...
			mockFhirClient.POST.mockResolvedValue({
				data: createdPatient,
				error: null,
				response: new Response(JSON.stringify(createdPatient), { status: 201 }),
			})
			...
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({ status: 'success', action: `cerbos:create` })
			)
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({
					status: 'success',
					action: toolName,
					targetResourceId: 'new-patient-id',
				})
			)
		})
		...
		it('should throw error and log when Cerbos authorization fails for create', async () => {
			...
			await expect(patientCreateTool.handler(createParams, mockCallContext)).rejects.toThrow(
				/Forbidden: User .* not authorized/
			)
		...
		it('should throw error and log when FHIR client create call fails', async () => {
			...
			const operationOutcome: OperationOutcome = {
				...
				issue: [{ severity: 'error', code: 'processing', diagnostics: 'Validation error' }],
			}
			mockFhirClient.POST.mockResolvedValue({
				data: null,
				error: fhirError,
				response: new Response(JSON.stringify(operationOutcome), { status: 400 }),
			})
			...
			expect(logAuditEvent).toHaveBeenCalledWith(
				expect.objectContaining({
					status: 'failure',
					action: toolName,
					outcomeDescription: 'Validation error',
				})
			)
		})
	})
