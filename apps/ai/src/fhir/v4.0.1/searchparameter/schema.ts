...
export function createSearchParameterSchema<C extends z.ZodTypeAny = z.ZodUnknown>(options?: {
	contained?: C
	...
	return getCachedSchema('SearchParameter', [contained], () => {
		const baseSchema: z.ZodType<types.SearchParameter<z.infer<C>>> = z.strictObject({
			...
			comparator: z.enum(['eq', 'ne', 'gt', 'lt', 'ge', 'le', 'sa', 'eb', 'ap']).array().optional(),
...
export function createSearchParameterComponentSchema() {
	return getCachedSchema('SearchParameterComponent', [], () => {
		const baseSchema: z.ZodType<types.SearchParameterComponent> = z.strictObject({
