...
import { TaggedLogger, LogLevel } from 'workers-tagged-logger'; // Assuming this can be used in Node.js
                                                              // or replace with another logger like pino or winston

import type { AuditLogEvent, AuditEventStatus } from '@repo/audit';

import { db, checkDbConnection } from './db';
import { auditLog as auditLogTableSchema } from './db/schema';
...
if (!REDIS_URL) {
	console.error('🔴 REDIS_URL environment variable is not set. Please check your .env file or environment configuration.');
	process.exit(1);
}
...
async function main() {
	...
	const processJob = async (job: Job<AuditLogEvent, any, string>): Promise<void> => {
		...
		try {
			...
			logger.info(`✅ Job ${job.id} processed successfully. Audit log for action '${action}' stored.`);
		} catch (error) {
